function [f,f_plot,f_error,initParam]=prepareUniModel(x,y,x_plot,selectedModel,optimMethod)
   f_plot=0;f_error=0;
   ...
   switch (selectedModel)
     case 1 %Equation 1
       ...
       initParam(1)=exp(coef(1)); initParam(2)=-coef(2);
     case 2 %Equation 2
      if (optimMethod==1)
      ...
      initParam(1)=exp(coef(1)); initParam(2)=-coef(2); initParam(3)=coef(3);
     case 3 %Equation 3
      if (optimMethod==1)
      ...
      coef=flip(polyfit(x,log(y),2));
      initParam(1)=exp(coef(1)); initParam(2)=-coef(2); initParam(3)=-coef(3)/coef(2);
     case 4 %Equation 4
      if (optimMethod==1)
     ...
     case 5 %Equation 5
      ...
      initParam(1)=coef(1); initParam(2)=1;
     case 6 %Equation 6
      if (optimMethod==1)
       f=@(x,p) (p(1)+p(2).*x).^(-p(3));
      else
      ...
      initParam(1:2)=coef; initParam(3)=1;
     case 7 %Equation 7
      if (optimMethod==1)
      ...
      initParam(1)=exp(coef(1)); initParam(2)=-coef(2);initParam(3)=1.1;
     case 8 %Equation 8
      if (optimMethod==1)
      f=@(x,p) p(1)./(1+p(2).*x);
      else
      ...
      coef=flip(polyfit(x,1./y,1));
      initParam(1)=1/coef(1);initParam(2)=coef(2)*initParam(1);
     otherwise

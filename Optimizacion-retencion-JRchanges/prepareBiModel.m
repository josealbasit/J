function [f,f_plot,f_error,initParam]=prepareBiModel(x,x1,x2,z,w,X,Y,selectedModel,optimMethod)
 ...
 switch (selectedModel) %Choosing the model...
    ...
    %Equation 1
      ...
      initParam(1)=inv(coef(1)); initParam(2)=coef(2)*initParam(1); initParam(3)=coef(3)*initParam(1); initParam(4)=coef(4)*initParam(1);
    case 2
    %Equation 2
      ...
      initParam(1)=inv(coef(1)); initParam(2)=coef(2)*initParam(1); initParam(3)=coef(3)*initParam(1); initParam(4)=coef(4)*initParam(1); initParam(5)=coef(5)*initParam(1);
    case 3
    %Equation 3
      ...
      initParam(1)=1000; initParam(2)=0.01; initParam(3)=100; initParam(4)=0.2;
    case 4
    %Equation4
      ...
      coef=(regress(1./z,[w x1 x2 x1.*x2]));
      initParam(1)=inv(coef(1)); initParam(2)=coef(2)*initParam(1); initParam(3)=coef(3)*initParam(1); initParam(4)=[coef(4)*initParam(1)]/initParam(3); initParam(5)=0;
      %initParam(1)=1000; initParam(2)=0.01; initParam(3)=100; initParam(4)=0.2; initParam(5)=-1;
    case 5
    %Equation 5
      ...
      initParam(1)=inv(coef(1)); initParam(2)=coef(3)*initParam(1); initParam(3)=coef(4)*initParam(1); initParam(4)=coef(2)*initParam(1); initParam(5)=[coef(5)*initParam(1)]/initParam(4); initParam(6)=0;
    case 6
    %Equation 6
      ...
      coef=(regress(1./z,[w x1 x2]));
      initParam(1)=inv(coef(1)); initParam(2)=coef(2)*coef(1); initParam(3)=coef(3)*coef(1); initParam(4:6)=[0 0 0];
  endswitch
  close;
